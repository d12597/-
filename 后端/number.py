...
def number(x,y):
    ...
    inlier=[]
    outlier=[]
    for i in range(len(x[inlier_mask])):
        inlier.append([x[inlier_mask][i][0],y[inlier_mask][i]])
    for i in range(len(x[outlier_mask])):
        outlier.append([x[outlier_mask][i][0],y[outlier_mask][i]])
    ...
    l2=[]
    for i in range(len(xx)):
        l2.append([xx[i],liner_quadratic.predict(xx_quadratic)[i]])
    # plt.plot(xx, liner_quadratic.predict(xx_quadratic), 'c-')
    # plt.plot(xx, liner_quadratic.predict(xx_quadratic), color='c', linewidth=lw, label='一元二次线性回归')
    
    l3=[]
    for i in range(len(line_x)):
        l3.append([line_x[i][0],line_y[i]])
    l4=[]
    for i in range(len(line_x)):
        l4.append([line_x[i][0],line_y_ransac[i]])
    # plt.plot(line_x, line_y, color='navy', linewidth=lw, label='线性回归')
    # plt.plot(line_x, line_y_ransac, color='r', linewidth=lw,label='RANSAC regressor')
